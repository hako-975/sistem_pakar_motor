...
<body class="layout-fixed sidebar-expand-lg bg-body-tertiary">
    <?php 
        if (isset($_POST['btnSpkJurusan'])) {
            ...
            if (!$error) {
                $log_berhasil = mysqli_query($conn, "INSERT INTO log VALUES ('', 'SPK Jurusan $nama_siswa Berhasil diubah!', CURRENT_TIMESTAMP(), " . $dataUser['id_user'] . ")");
                echo "
                    <script>
                        Swal.fire({
                            icon: 'success',
                            title: 'Berhasil!',
                            text: 'SPK Jurusan " . $nama_siswa . " berhasil diubah!'
                        }).then((result) => {
                            if (result.isConfirmed) {
                ...
                echo "
                    <script>
                        Swal.fire({
                            icon: 'error',
                            title: 'Gagal!',
                            text: 'Jurusan " . $nama_siswa . " gagal diubah!'
                        }).then((result) => {
                            if (result.isConfirmed) {
                                window.history.back();
                            }
                                                ...
                                                <?php foreach ($kriteria as $dk): ?>
                                                    ...
                                                    <div class="mb-3 col">
                                                        <label for="nilai_<?= $de['id_jurusan']; ?>_<?= $dk['id_kriteria']; ?>" class="form-label">
                                                            <?= htmlspecialchars($dk['nama_kriteria']); ?> Jurusan <?= htmlspecialchars($de['nama_jurusan']); ?> (0-10)
                                                        </label>
                                                        <?php 
                                                            $id_jurusan = $de['id_jurusan'];
                                                            $id_kriteria = $dk['id_kriteria'];
                                                            $penilaian = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM penilaian WHERE id_hasil = '$id_hasil' AND id_jurusan = '$id_jurusan' AND id_kriteria = '$id_kriteria'"));
                                                        ?>
                                                        <input type="number" step="0.01" id="nilai_<?= $de['id_jurusan']; ?>_<?= $dk['id_kriteria']; ?>" class="form-control" name="penilaian[<?= $de['id_jurusan']; ?>][<?= $dk['id_kriteria']; ?>][nilai]" min="0" max="10" value="<?= $penilaian['nilai']; ?>" required>
                                                    </div>
                                                <?php endforeach; ?>
                                            </div>
                                            <hr>
